...
const AddQuestion = ({ question, questionIndex }) => {
  ...
  const handleDeleteQuestion = (e) => {
    e.preventDefault();
    deleteQuestion(questionItem, questionIndex);
  }

  return (
    <section className = "mb-3 mt-3">
      {!editMode && saveMode && (
      <article className = {`${styles.questionSaved}`}>
        <span className = {`${styles.questionSavedNumber}`}>{questionIndex + 1}</span>
        <span className = {`${styles.questionSavedTitle} ml-3`}>{question?.title}</span>
      ...
      {editMode && !saveMode && (
      <section className = {`${styles.questionForm} mb-5 mt-5`}>
        <Button className = {`btn ${styles.closeBtn}`} variant = "dark" onClick = {(e) => handleDeleteQuestion(e)}>
          <Close />
        </Button>
          ...
          return (
            <InputGroup className ="mb-4" key = {index}>
              <InputGroup.Prepend>
                <Button 
                  variant = "success" 
                  className = {`${styles.optionBtn} ${questionItem?.correctIndex === index && `${styles.correctOption}`}`} 
